...
//请补充代码
    ...
    result = mergeArrayToInt(numArray) ^ Rcon[round];
...
void addRoundKey(int array[4][4], int round)
{
  ...
  //请补充代码
    ...
    for(int i = 0;i < 4;i++)
    {
        splitIntToArray(w[i + round * 4],wRoundArray);
        for(int j = 0;j < 4;j++)
        {
            array[j][i] = array[j][i] ^ wRoundArray[j];
        }
    }
}
...
*/
    int arrayRow2[4],arrayRow3[4],arrayRow4[4];
    for(int i = 0;i < 4;i++)
    {
        arrayRow2[i] = array[1][i];
        arrayRow3[i] = array[2][i];
        arrayRow4[i] = array[3][i];
    }

    leftLoop4int(arrayRow2,1);
    leftLoop4int(arrayRow3,2);
    leftLoop4int(arrayRow4,3);

    for(int i = 0;i < 4;i++)
    {
        array[1][i] = arrayRow2[i];
        array[2][i] = arrayRow3[i];
        array[3][i] = arrayRow4[i];
    }
...
//请补充代码
    ...
    for(int i = 0;i < 4;i++)
    {
        for(int j = 0;j < 4;j++)
        {
            array[i][j] = GFMul(colM[i][0],tmp[0][j])^GFMul(colM[i][1],tmp[1][j])^GFMul(colM[i][2],tmp[2][j])^GFMul(colM[i][3],tmp[3][j]);
        }
    }
...
void deShiftRows(int array[4][4])
{
    ...
    rightLoop4int(arrayRow2,1);
    rightLoop4int(arrayRow3,2);
    rightLoop4int(arrayRow4,3);
...
//请补充代码
    ...
    for(int i = 0;i < 4;i++)
    {
        for(int j = 0;j < 4;j++)
        {
            array[i][j] = GFMul(deColM[i][0],tmp[0][j])^GFMul(deColM[i][1],tmp[1][j])^GFMul(deColM[i][2],tmp[2][j])^GFMul(deColM[i][3],tmp[3][j]);
        }
    }
}
/**
